...
DOCUMENTATION = """
---
module: staros_command
version_added: "2.1"
author: "Peter Sprygada (@privateip)"
short_description: Run commands on remote devices running Cisco IOS
description:
  - Sends arbitrary commands to an staros node and returns the results
    read from the device. This module includes an
    argument that will cause the module to wait for a specific condition
    before returning or timing out if the condition is not met.
  - This module does not support running commands in configuration mode.
    Please use M(staros_config) to configure IOS devices.
...
options:
  commands:
    description:
      - List of commands to send to the remote staros device over the
...
from ansible.module_utils.network.staros.staros import run_commands
