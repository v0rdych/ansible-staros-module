...
# that is returned from the `staros_facts` module
...
backup_path:
  ...
  type: str
  sample: /playbooks/ansible/backup/staros_config.2016-07-16@22:28:34
filename:
  description: The name of the backup file
  returned: when backup is yes and filename is not specified in backup options
  type: str
  sample: staros_config.2016-07-16@22:28:34
shortname:
  description: The full path to the backup file excluding the timestamp
  returned: when backup is yes and filename is not specified in backup options
  type: str
  sample: /playbooks/ansible/backup/staros_config
date:
...
from ansible.module_utils.network.staros.staros import run_commands, get_config
from ansible.module_utils.network.staros.staros import get_defaults_flag, get_connection
from ansible.module_utils.network.staros.staros import staros_argument_spec
from ansible.module_utils.network.staros.staros import check_args as staros_check_args
from ansible.module_utils.basic import AnsibleModule
from ansible.module_utils.network.common.config import NetworkConfig, dumps


def check_args(module, warnings):
    staros_check_args(module, warnings)
...
def main():
    ...
    if module.params['save_when'] == 'always':
        ...
        output = run_commands(module, ['show configuration ', 'show configuration '])

        running_config = NetworkConfig(indent=1, contents=output[0], ignore_lines=diff_ignore_lines)
        startup_config = NetworkConfig(indent=1, contents=output[1], ignore_lines=diff_ignore_lines)

        if running_config.sha1 != startup_config.sha1:
            result['tstrun'] = running_config;
            result['tststart'] = startup_config;
            save_config(module, result)
